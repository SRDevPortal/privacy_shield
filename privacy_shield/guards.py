...
def preserve_derived(submitted, stored, fields):
    """Restore backend-derived values; they can contain legitimate non-phone text."""
    result = deepcopy(submitted)
    for field in fields:
        if field not in submitted:
            if field in stored:
                result[field] = stored[field]
        elif submitted[field] != stored.get(field):
            raise PermissionError("Derived values must be generated by the server")
    return result
