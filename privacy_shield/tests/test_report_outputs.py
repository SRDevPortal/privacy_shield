...
class ReportOutputTests(unittest.TestCase):
    ...
    def test_custom_report_reference_cannot_bypass(self):
        ...
        with patch.object(frappe,"get_doc",side_effect=docs),patch(PREFIX+"enqueue_json_to_csv_conversion") as original:
    ...
    def test_conversion_rechecks_current_user_policy(self):
        ...
        with patch.object(frappe,"conf",{"privacy_shield_desk_enabled":True}),patch.object(frappe.local,"job",frappe._dict(user="agent"),create=True),patch.object(frappe,"session",frappe._dict(user="agent")),patch.object(reports,"check_prepared",side_effect=frappe.PermissionError) as check:
